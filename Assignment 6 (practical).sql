...
select 
rank() over(order by score desc) as rank_by_score,
student_id,score,name,course,attendance,mentor,join_date,city
...
score,student_id,name,course,attendance,mentor,join_date,city
from student_performance;

## Question 3 : Convert all student names to uppercase and extract the month name from join_date.

select
upper(name),monthname(join_date)
from student_performance;
...
select
name,attendance,
lead(attendance) over(order by attendance asc) as nxt_stud_attendence,
score,join_date,course,mentor
from student_performance;
...
select
row_number() over(partition by course order by attendance desc) as rank_course,
course,score,name,attendance
from student_performance;
...
select
name,
datediff('2025-01-01',join_date) no_of_days
from student_performance;
...
name,
city,
replace(city,"Mumbai", "MUM") dis_city
from student_performance;
